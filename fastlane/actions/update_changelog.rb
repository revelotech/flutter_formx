  ...
  module Actions
    ...
    class UpdateChangelogAction < Action
      def self.run(params)
        ...
        unversioned_title = '## Changelog'

        file = File.read(file_path)
        split_file = file.split(unversioned_title)
        return unless split_file.length == 2
          ...
          FastlaneCore::ConfigItem.new(key: :release_name,
                                       ...
                                       verify_block: proc do |value|
                                         unless value and !value.empty?
                                           UI.user_error!("No release name for UpdateChangelogAction given, pass using `release_name: 'name'`")
                                         end
